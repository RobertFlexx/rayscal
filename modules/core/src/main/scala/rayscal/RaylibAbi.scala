...
object RaylibAbi:
  ...
  def validate(): Unit =
    if !validated then
      ...
      check("Color", sizeof[raw.Color].toInt, RayscalNative.SizeOfColor())
      check("Vector2", sizeof[raw.Vector2].toInt, RayscalNative.SizeOfVector2())
      check("Vector3", sizeof[raw.Vector3].toInt, RayscalNative.SizeOfVector3())
      ...
      check("Rectangle", sizeof[raw.Rectangle].toInt, RayscalNative.SizeOfRectangle())
      ...
      check("Camera2D", sizeof[raw.Camera2D].toInt, RayscalNative.SizeOfCamera2D())
      check("Camera3D", sizeof[raw.Camera3D].toInt, RayscalNative.SizeOfCamera3D())
      check("Ray", sizeof[raw.Ray].toInt, RayscalNative.SizeOfRay())
      check("RayCollision", sizeof[raw.RayCollision].toInt, RayscalNative.SizeOfRayCollision())
      check("BoundingBox", sizeof[raw.BoundingBox].toInt, RayscalNative.SizeOfBoundingBox())
  ...
  private def validateStructLayout(): Unit =
    Zone:
      val color = alloc[raw.Color]()
      (!color)._1 = 11.toUByte
      (!color)._2 = 22.toUByte
      (!color)._3 = 33.toUByte
      (!color)._4 = 44.toUByte

      val camera = alloc[raw.Camera2D]()
      (!camera)._1 = 1.25f
      (!camera)._2 = -2.5f
      (!camera)._3 = 3.75f
      (!camera)._4 = -4.5f
      ...
      val stream = alloc[raw.AudioStream]()
      (!stream)._1 = null
      (!stream)._2 = null
      (!stream)._3 = 48000.toUInt
      (!stream)._4 = 32.toUInt
      (!stream)._5 = 2.toUInt
      val sound = alloc[raw.Sound]()
      (!sound)._1 = !stream
      (!sound)._2 = 777.toUInt
